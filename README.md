...
- 直接上传 PDF 谱图，自动拆页、定位谱行和小节，并把 PDF 六线谱送入同一套可播放校对流程；
...
- 像 Guitar Pro 一样编辑三连音、多声部和任意复杂结构（当前已支持逐小节拍号与基础附点时值）；
...
六线网格编辑器使用独立光标：`↑/↓` 只切换 1～6 弦，`←/→` 按当前输入时值前后移动，`0–9` 只写入当前弦，`Delete/Backspace` 把最后一个音变为等时值实体休止。同一列可逐弦组成和弦；在已有延音内部输入时，会在游标位置切开前一个节拍。空白拍位先显示透明休止和输入框，`Enter` 或 `R` 可提交实体休止；手机和平板可用可见的品位输入框与方向键完成相同操作。
...
识别结果和手动空白谱共用逐小节的十六分网格：拖动或 `Shift + ←/→` 连续多选，`Ctrl/Cmd + 点击` 离散多选，数字键直接输入品位，`.` 切换单附点，`Ctrl/Cmd + .` 设置双附点。每小节按自己的拍号计算容量；音符与实体休止未连续填满、互相重叠或越过小节线时整小节标红。识别项目点击“重试本小节识别”只生成未保存提案，不会直接覆盖已经校正的版本。
...
- 品位数字清晰，透视、旋转和遮挡较少；
- 默认标准六弦调弦和 4/4 拍；可逐小节改拍号，自动节奏会尝试区分单梁八分与双梁十六分，校对器当前使用十六分位置网格；
- OCR 仍会用相邻事件间隙提出隐式休止；用户必须在校对器里将它们确认为实体休止后，小节才算拍数完整。复杂连休止、三连音和被遮挡的符干仍需要人工校正；
- 重复小节可用于投票纠错，而不是直接全部删除。

PDF 上传会保留原文件，同时生成逐页预览和仅供识别使用的谱行裁图。当前限制为 40 MB、80 页；带密码 PDF、手写谱和严重倾斜扫描件会直接提示人工处理。对本仓库的真实 8 页回归样本《藍二乗》，布局阶段应稳定得到 69 行谱和 206 个小节。

联合布局目前由下方六线谱提供弦与品位，上方五线谱用于小节号、布局判断、节奏参考和完整 PDF 裁切；复杂五线谱节奏仍要在十六分网格中校正。H/P、`sl.`、`P.M.`、`Full/Bend` 与泛音文字会作为低风险候选写入草稿，纯波浪揉弦、复杂连音弧和三连音仍必须人工确认。识别结果会保存覆盖置信度、数字覆盖率、技巧计数、布局/极性、缺失小节、来源帧和解析警告。没有可靠小节号时不会再自动从 0 开始编号，系统也不会把低覆盖结果包装成确定的成品谱。
