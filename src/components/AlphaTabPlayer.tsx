...
type ScoreLasso = {
  ...
  active: boolean;
  additive: boolean;
  baseSelection: alphaTab.model.Note[];
};
...
function restGlyph(eighths: number) {
  return ({ 8: "𝄻", 4: "𝄼", 2: "𝄽", 1: "𝄾", 0.5: "𝄿" } as Record<number, string>)[eighths] ?? "𝄽";
}

function barRhythm(bar: alphaTab.model.Bar) {
  const capacity = bar.masterBar.timeSignatureNumerator * 8 / bar.masterBar.timeSignatureDenominator;
  const voice = bar.voices[0];
  const used = voice?.beats.reduce((sum, beat) => sum + (beat.isEmpty ? 0 : beatDurationEighths(beat)), 0) ?? 0;
  return { used, capacity, invalid: Math.abs(used - capacity) > 1e-9 };
}

function formatQuarterBeats(eighths: number) {
  return (eighths / 2).toFixed(1);
}
...
function restoreBeatState(state: BeatState) {
  ...
  state.beat.isEmpty = state.isEmpty;
}
...
export function AlphaTabPlayer({
  ...
  const scoreLassoRef = useRef<ScoreLasso | null>(null);
  ...
  const [dragSelectionBox, setDragSelectionBox] = useState<SelectionMarker | null>(null);
  ...
  function stringYForBeatBounds(bounds: alphaTab.rendering.BeatBounds, alphaString: number) {
    ...
    const samples = systemBars.flatMap((candidate) => candidate.beats.flatMap((beatBounds) =>
      (beatBounds.notes ?? []).map((noteBounds) => ({
        ...
        y: noteBounds.noteHeadBounds.y + noteBounds.noteHeadBounds.h / 2
      }))
    ));
    const grouped = new Map<number, number[]>();
    for (const sample of samples) grouped.set(sample.string, [...(grouped.get(sample.string) ?? []), sample.y]);
    const points = [...grouped].map(([string, values]) => ({ string, y: values.reduce((sum, value) => sum + value, 0) / values.length }));
    if (points.length >= 2) {
      const meanString = points.reduce((sum, point) => sum + point.string, 0) / points.length;
      const meanY = points.reduce((sum, point) => sum + point.y, 0) / points.length;
      const variance = points.reduce((sum, point) => sum + (point.string - meanString) ** 2, 0);
      const slope = variance > 0 ? points.reduce((sum, point) => sum + (point.string - meanString) * (point.y - meanY), 0) / variance : 0;
      if (slope <= -4 && slope >= -24) return meanY + slope * (alphaString - meanString);
    }
    if (points.length === 1) return points[0].y - 10 * (alphaString - points[0].string);
    const real = barBounds.realBounds;
    const visualString = stringCount + 1 - alphaString;
    if (real.h > 90) {
      const top = real.y + real.h * 0.58;
      const gap = Math.max(7, Math.min(13, real.h * 0.38 / Math.max(1, stringCount - 1)));
      return top + (visualString - 1) * gap;
    }
    return real.y + real.h * ((visualString - 0.5) / stringCount);
  }

  function beatColumnX(bounds: alphaTab.rendering.BeatBounds) {
    const { x, w } = bounds.realBounds;
    return Number.isFinite(bounds.onNotesX) && bounds.onNotesX >= x - 1 && bounds.onNotesX <= x + w + 1
      ? bounds.onNotesX
      : x + w / 2;
  }

  function beatStringRange(bounds: alphaTab.rendering.BeatBounds) {
    const stringCount = bounds.beat?.voice?.bar?.staff?.tuning.length || 6;
    const rows = Array.from({ length: stringCount }, (_, index) => stringYForBeatBounds(bounds, index + 1));
    return { top: Math.min(...rows), bottom: Math.max(...rows), stringCount };
  }

  function cursorMarkerFor(bounds: alphaTab.rendering.BeatBounds, string: number, kind: ScoreCursor["kind"]) {
    const host = hostRef.current;
    if (!host) return null;
    const range = beatStringRange(bounds);
    const y = kind === "rest" ? (range.top + range.bottom) / 2 : stringYForBeatBounds(bounds, string);
    return {
      id: -1,
      x: host.offsetLeft + beatColumnX(bounds) - 9,
      y: host.offsetTop + y - 9,
      width: 18,
      height: 18
  ...
  function updateSelectionMarkers() {
    ...
    if (currentCursor) {
      const bounds = lookup.findBeat(currentCursor.beat);
      const marker = bounds ? cursorMarkerFor(bounds, currentCursor.string, currentCursor.kind) : null;
      if (marker) {
        const next = { ...currentCursor, marker };
    ...
    if (!staff) setMeasureMarkers([]);
    else setMeasureMarkers(staff.bars.flatMap((bar, index) => {
      const { used, capacity, invalid } = barRhythm(bar);
      if (!invalid) return [];
      const measure = firstMeasure + index;
      const masterBounds = lookup.findMasterBarByIndex(index);
  ...
  function setEntryDuration(value: number) {
    ...
    if (scoreCursorRef.current) setEditStatus(`灰色休止预览已切换为 ${directDurationLabel(value)} · 输入品位写音符，Enter 确认休止`);
  }

  function measureNumberForBeat(beat: alphaTab.model.Beat) {
    return (recognitionRef.current?.summary.start_measure ?? 1) + beat.voice.bar.masterBar.index;
  }
  ...
  function selectScoreCellFromPointer(event: PointerEvent) {
    ...
    const stringRows = Array.from({ length: stringCount }, (_, index) => ({
      ...
      y: stringYForBeatBounds(beatBounds, stringCount - index)
    }));
    const visualString = [...stringRows].sort((left, right) => Math.abs(left.y - y) - Math.abs(right.y - y))[0].visualString;
    const alphaString = stringCount + 1 - visualString;
    const beatStart = Math.round(beat.playbackStart / EIGHTH_NOTE_TICKS * 2) / 2;
    const onset = beatStart;
    const marker = cursorMarkerFor(beatBounds, alphaString, "string");
    if (!marker) return;
    const cursor = { beat, onset, string: alphaString, kind: "string" as const, marker };
    scoreCursorRef.current = cursor;
    setScoreCursor(cursor);
    setSelection([]);
    const measure = measureNumberForBeat(beat);
    ...
    if (beat.isRest && !beat.isEmpty) {
      ...
      setEditStatus(`第 ${measure} 小节 · 实体休止 ${directDurationLabel(duration)} · 可改时值或 Delete 变灰`);
  ...
  function handleNotePointerDown(note: alphaTab.model.Note) {
    ...
    clearScoreCursor();
    entryDurationRef.current = beatDurationEighths(note.beat);
    setEntryDurationState(entryDurationRef.current);
  ...
  function handleNotePointerMove(note: alphaTab.model.Note) {
    if (editingDisabledRef.current || savingRef.current) return;
    if (scoreLassoRef.current) return;
  ...
  useEffect(() => {
    if (!hostRef.current || !scrollElement) return;
    const host = hostRef.current;
    ...
    apiRef.current = api;
    const pointInHost = (event: PointerEvent | MouseEvent) => {
      const bounds = host.getBoundingClientRect();
      return {
        x: Math.max(0, Math.min(host.offsetWidth, event.clientX - bounds.left)),
        y: Math.max(0, Math.min(host.offsetHeight, event.clientY - bounds.top))
      };
    };
    const selectNotesInLasso = (lasso: ScoreLasso) => {
      const score = scoreRef.current;
      const lookup = api.renderer.boundsLookup;
      ...
      const beats = flattenBeats(score).filter((beat) => {
        if (!beat.notes.length) return false;
        const bounds = lookup.findBeat(beat);
        if (!bounds) return false;
        const range = beatStringRange(bounds);
        const column = beatColumnX(bounds);
        return column >= left && column <= right && range.bottom >= top && range.top <= bottom;
      });
      const notes = beats.flatMap((beat) => beat.notes);
      const next = lasso.additive ? uniqueNotes([...lasso.baseSelection, ...notes]) : uniqueNotes(notes);
      setSelection(next, next.at(-1) ?? null, true);
      setEditStatus(next.length ? `框选 ${beats.length} 个节奏列 · 共 ${next.length} 个音` : "框选范围内没有可编辑音符");
    };
    const capturePointerDown = (event: PointerEvent) => {
      ...
      scoreLassoRef.current = {
        ...
        baseSelection: [...selectedNotesRef.current]
      };
      try { host.setPointerCapture(event.pointerId); } catch { /* Pointer capture is optional. */ }
    };
    const capturePointerMove = (event: PointerEvent) => {
      const lasso = scoreLassoRef.current;
      if (!lasso || lasso.pointerId !== event.pointerId) return;
      const point = pointInHost(event);
      lasso.currentX = point.x;
      lasso.currentY = point.y;
      if (!lasso.active && Math.hypot(point.x - lasso.startX, point.y - lasso.startY) >= 6) {
        lasso.active = true;
        clearScoreCursor();
      }
      ...
      setDragSelectionBox({
        id: -2,
        x: host.offsetLeft + Math.min(lasso.startX, point.x),
        y: host.offsetTop + Math.min(lasso.startY, point.y),
        width: Math.abs(point.x - lasso.startX),
        height: Math.abs(point.y - lasso.startY)
      });
    };
    const finishPointer = (event: PointerEvent) => {
      ...
      if (lasso.active) {
        event.preventDefault();
        selectNotesInLasso(lasso);
      } else selectScoreCellFromPointer(event);
      scoreLassoRef.current = null;
      setDragSelectionBox(null);
      try { host.releasePointerCapture(event.pointerId); } catch { /* Pointer capture is optional. */ }
    };
    const cancelPointer = (event: PointerEvent) => {
      if (scoreLassoRef.current?.pointerId !== event.pointerId) return;
      scoreLassoRef.current = null;
      setDragSelectionBox(null);
    };
    const openRestColumn = (event: MouseEvent) => {
      ...
      event.preventDefault();
      const stringCount = beat.voice.bar.staff.tuning.length || 6;
      const string = Math.max(1, Math.ceil(stringCount / 2));
      const marker = cursorMarkerFor(bounds, string, "rest");
      if (!marker) return;
      ...
      setFocusedMeasure(measure);
      onFocusMeasureRef.current(measure);
      setEditStatus(beat.isEmpty
        ? `第 ${measure} 小节 · 透明休止 ${directDurationLabel(duration)} · Enter 恢复实体`
        : `第 ${measure} 小节 · 实体休止 ${directDurationLabel(duration)} · Delete 变透明`);
    };
    host.addEventListener("pointerdown", capturePointerDown, true);
    host.addEventListener("pointermove", capturePointerMove, true);
    ...
    return () => {
      ...
      host.removeEventListener("pointermove", capturePointerMove, true);
      host.removeEventListener("pointerup", finishPointer, true);
  ...
  function recognitionEventsWithInsertedNote(
    ...
    for (const event of measure.events) {
      ...
      if (!event.rest) {
        ...
        if (leading <= 1e-9 || trailing < -1e-9) return null;
        events.push({ ...event, duration_eighths: leading, notes: event.notes.map((note) => ({ ...note })) });
  ...
  function newBeatForEdit(voice: alphaTab.model.Voice, entry: HistoryEntry, index: number, duration: alphaTab.model.Duration, dots = 0) {
    ...
    beat.dots = dots;
    beat.isEmpty = false;
    entry.beatStates.push({ beat, voice, index, present: false, duration: beat.duration, dots: beat.dots, isEmpty: beat.isEmpty });
    return beat;
  }

  function newEmptyBeatForEdit(voice: alphaTab.model.Voice, entry: HistoryEntry, index: number, duration: alphaTab.model.Duration, dots = 0) {
    const beat = newBeatForEdit(voice, entry, index, duration, dots);
    beat.isEmpty = true;
    entry.beatStates[entry.beatStates.length - 1].isEmpty = true;
    return beat;
  }

  function resizeRestBeatWithGhostRemainder(beat: alphaTab.model.Beat, eighths: number, entry: HistoryEntry) {
    const voice = beat.voice;
    const beatIndex = voice.beats.indexOf(beat);
    if (beatIndex < 0) return false;
    const current = beatDurationEighths(beat);
    const delta = eighths - current;
    if (Math.abs(delta) < 1e-9 && beat.dots === 0) {
      rememberBeat(entry, beat);
      return true;
    }
    let end = beatIndex + 1;
    while (end < voice.beats.length && voice.beats[end].isEmpty) end += 1;
    const emptyBeats = voice.beats.slice(beatIndex + 1, end);
    const explicitEmpty = emptyBeats.reduce((sum, candidate) => sum + beatDurationEighths(candidate), 0);
    ...
    const implicitEmpty = end === voice.beats.length ? Math.max(0, capacity - timeline) : 0;
    if (delta > explicitEmpty + implicitEmpty + 1e-9) return false;

    rememberBeat(entry, beat);
    beat.duration = alphaDuration(eighths);
    beat.dots = 0;
    const targetExplicitEmpty = delta >= 0
      ? Math.max(0, explicitEmpty - Math.max(0, delta - implicitEmpty))
      : explicitEmpty - delta;
    const shapes = restShapes(targetExplicitEmpty);
    for (const empty of emptyBeats) rememberBeat(entry, empty);
    for (let index = 0; index < shapes.length; index += 1) {
      const shape = shapes[index];
      let empty = emptyBeats[index];
      if (!empty) {
        empty = newEmptyBeatForEdit(voice, entry, beatIndex + 1 + index, shape.duration, shape.dots);
        voice.beats.splice(beatIndex + 1 + index, 0, empty);
      }
      empty.duration = shape.duration;
      empty.dots = shape.dots;
      empty.isEmpty = true;
    }
    for (const empty of emptyBeats.slice(shapes.length)) {
      const index = voice.beats.indexOf(empty);
  ...
  function insertFretAtScoreCursor(digit: string) {
    ...
    } else if (splittingBeat) {
      const originalIndex = voice.beats.indexOf(beat);
      const leading = cursor.onset - beatStart;
      const leadingShape = REST_DURATION_SHAPES.find((shape) => Math.abs(shape.units - leading) < 1e-9);
      if (originalIndex < 0 || !leadingShape) {
        setEditStatus("这个位置无法按当前节奏安全切分");
        return true;
      }
      beat.duration = leadingShape.duration;
      beat.dots = leadingShape.dots;
      noteBeat = newBeatForEdit(voice, entry, originalIndex + 1, alphaDuration(duration));
      const replacements = [noteBeat];
      const trailing = Math.max(0, beatEnd - cursor.onset - duration);
      for (const shape of restShapes(trailing)) replacements.push(newBeatForEdit(voice, entry, originalIndex + 1 + replacements.length, shape.duration, shape.dots));
      voice.beats.splice(originalIndex + 1, 0, ...replacements);
  ...
  function cursorAt(beat: alphaTab.model.Beat, onset: number, string: number, kind: ScoreCursor["kind"] = "string") {
    const api = apiRef.current;
    const host = hostRef.current;
    ...
    if (!bounds) {
      ...
      if (current?.beat !== beat) return null;
      return {
        beat,
        onset,
        string,
        kind,
        marker: { ...current.marker, y: current.marker.y + (current.string - string) * 10 }
      } satisfies ScoreCursor;
    }
    const marker = cursorMarkerFor(bounds, string, kind);
  ...
  function adjacentVisibleBeat(origin: alphaTab.model.Beat, direction: -1 | 1) {
    const staff = origin.voice.bar.staff;
    const voiceIndex = origin.voice.index;
    const beats = staff.bars.flatMap((bar) => (bar.voices[voiceIndex] ?? bar.voices[0])?.beats ?? []);
  ...
  function activateScorePosition(beat: alphaTab.model.Beat, onset: number, string: number, status: string) {
    const beatStart = Math.round(beat.playbackStart / EIGHTH_NOTE_TICKS * 2) / 2;
    const note = Math.abs(onset - beatStart) < 1e-9 ? beat.getNoteOnString(string) : null;
    if (note) {
      clearScoreCursor();
      setSelection([note], note);
      return;
  ...
  function moveScoreCursor(horizontal: number, vertical: number) {
    ...
    if (current.kind === "rest" && vertical) return setEditStatus("休止符属于整列，不区分 1–6 弦");
    const string = Math.max(1, Math.min(stringCount, current.string - vertical));
    ...
    activateScorePosition(beat, onset, string, `${visualString} 弦 · 灰色休止预览 ${directDurationLabel(entryDurationRef.current)} · 输入品位或 Enter 确认`);
  }
  ...
  function selectCursorString(visualString: number) {
    ...
    scoreCursorRef.current = next;
    setScoreCursor(next);
    setEditStatus(current.beat.isRest && !current.beat.isEmpty
      ? `${visualString} 弦 · 实体休止 ${directDurationLabel(beatDurationEighths(current.beat))} · 可改时值或 Delete 变灰`
      : `${visualString} 弦 · 灰色休止预览 ${directDurationLabel(entryDurationRef.current)} · 输入品位或 Enter 确认`);
  }
  ...
  function commitRestAtScoreCursor() {
    ...
    const beat = cursor.beat;
    const measure = recognitionMeasureForBeat(beat);
    const measureNumber = measureNumberForBeat(beat);
    const duration = entryDurationRef.current;
    const beatStart = Math.round(beat.playbackStart / EIGHTH_NOTE_TICKS * 2) / 2;
    if (beat.isRest && Math.abs(cursor.onset - beatStart) < 1e-9) {
      if (!beat.isEmpty) {
        setEditStatus("当前位置已经是实体休止符；按 Delete 可退回灰色预览");
        return true;
      }
      const capacity = beat.voice.bar.masterBar.timeSignatureNumerator * 8 / beat.voice.bar.masterBar.timeSignatureDenominator;
      const nextEvent = measure?.events
        .filter((event) => event.onset_eighths > cursor.onset)
        .sort((left, right) => left.onset_eighths - right.onset_eighths)[0];
      if (cursor.onset + duration > (nextEvent?.onset_eighths ?? capacity) + 1e-9) {
        setEditStatus("这个休止时值会覆盖后一个节拍，先缩短时值");
        return true;
      }
      const entry = historyEntry("确认休止", []);
      if (!resizeRestBeatWithGhostRemainder(beat, duration, entry)) {
        setEditStatus(`第 ${measureNumber} 小节没有足够灰色空拍容纳 ${directDurationLabel(duration)}`);
        return true;
      }
      beat.isEmpty = false;
      if (measure) {
        ...
        else measure.events.push({ onset_eighths: cursor.onset, duration_eighths: duration, notes: [], rest: true });
        measure.events.sort((left, right) => left.onset_eighths - right.onset_eighths);
        dirtyRecognitionMeasuresRef.current.add(measure.number);
      }
      clearScoreCursor();
      commitEdit(`第 ${measureNumber} 小节休止已实体化`, entry, []);
      return true;
    }
    const splittingBeat = !beat.isRest && cursor.onset > beatStart + 1e-9;
    if (!beat.isRest && !splittingBeat) return false;
    const beatEnd = beatStart + beatDurationEighths(beat);
    ...
    for (const event of measure?.events ?? []) {
      ...
      if (!event.rest) {
        const leading = cursor.onset - event.onset_eighths;
        const trailing = eventEnd - end;
        if (!splittingBeat || leading <= 1e-9 || trailing < -1e-9) {
    ...
    if (measure) events.push({ onset_eighths: cursor.onset, duration_eighths: duration, notes: [], rest: true });
    const entry = historyEntry("写入休止", splittingBeat ? beat.notes : []);
    const voice = beat.voice;
    const originalIndex = voice.beats.indexOf(beat);
    if (splittingBeat) {
      const leading = cursor.onset - beatStart;
      const leadingShape = REST_DURATION_SHAPES.find((shape) => Math.abs(shape.units - leading) < 1e-9);
      if (originalIndex < 0 || !leadingShape) return true;
      beat.duration = leadingShape.duration;
      beat.dots = leadingShape.dots;
      const replacements: alphaTab.model.Beat[] = [newBeatForEdit(voice, entry, originalIndex + 1, alphaDuration(duration))];
      for (const shape of restShapes(beatEnd - end)) replacements.push(newBeatForEdit(voice, entry, originalIndex + 1 + replacements.length, shape.duration, shape.dots));
      voice.beats.splice(originalIndex + 1, 0, ...replacements);
    } else {
      rememberBeat(entry, beat);
      voice.beats.splice(originalIndex, 1);
  ...
  function setRestAtScoreCursorDuration(eighths: number) {
    const cursor = scoreCursorRef.current;
    const beat = cursor?.beat;
    if (!cursor || !beat?.isRest || beat.isEmpty) return false;
    const measureNumber = measureNumberForBeat(beat);
    const measure = recognitionMeasureForBeat(beat);
    if (Math.abs(beatDurationEighths(beat) - eighths) < 1e-9 && beat.dots === 0) {
      setEditStatus(`实体休止已经是 ${directDurationLabel(eighths)}`);
      return true;
    }
    const beatStart = Math.round(beat.playbackStart / EIGHTH_NOTE_TICKS * 2) / 2;
    const capacity = beat.voice.bar.masterBar.timeSignatureNumerator * 8 / beat.voice.bar.masterBar.timeSignatureDenominator;
    const nextEvent = measure?.events
      .filter((event) => event.onset_eighths > beatStart)
      .sort((left, right) => left.onset_eighths - right.onset_eighths)[0];
    if (beatStart + eighths > (nextEvent?.onset_eighths ?? capacity) + 1e-9) {
      setEditStatus("这个休止时值会覆盖后一个节拍，未修改");
      return true;
    }
    const entry = historyEntry("休止时值", []);
    if (!resizeRestBeatWithGhostRemainder(beat, eighths, entry)) {
      setEditStatus(`后方灰色空拍不足，实体休止不能延长为 ${directDurationLabel(eighths)}`);
      return true;
    }
    if (measure) {
      const target = measure.events.find((event) => event.rest && Math.abs(event.onset_eighths - beatStart) < 1e-9);
      if (target) target.duration_eighths = eighths;
      else measure.events.push({ onset_eighths: beatStart, duration_eighths: eighths, notes: [], rest: true });
      measure.events.sort((left, right) => left.onset_eighths - right.onset_eighths);
      dirtyRecognitionMeasuresRef.current.add(measure.number);
    }
    entryDurationRef.current = eighths;
    setEntryDurationState(eighths);
    commitEdit(`第 ${measureNumber} 小节实体休止改为 ${directDurationLabel(eighths)}`, entry, []);
    return true;
  }
  ...
  function deleteRestAtScoreCursor() {
    const cursor = scoreCursorRef.current;
    const beat = cursor?.beat;
    if (!cursor || !beat?.isRest || beat.isEmpty) return false;
    const measure = recognitionMeasureForBeat(beat);
    const measureNumber = measureNumberForBeat(beat);
    const entry = historyEntry("删除休止", []);
    rememberBeat(entry, beat);
    const duration = beatDurationEighths(beat);
    const beatStart = Math.round(beat.playbackStart / EIGHTH_NOTE_TICKS * 2) / 2;
    beat.isEmpty = true;
    ...
    entryDurationRef.current = duration;
    setEntryDurationState(duration);
    commitEdit(`第 ${measureNumber} 小节休止已变为灰色预览，不计入小节拍数`, entry, []);
    return true;
  }
  ...
  function adjustSelectedBeatDuration(shorter: boolean) {
    if (scoreCursorRef.current) {
      const current = scoreCursorRef.current.beat.isRest && !scoreCursorRef.current.beat.isEmpty
        ? beatBaseDurationEighths(scoreCursorRef.current.beat)
      ...
      setScoreCursorDuration(DIRECT_DURATIONS[nextIndex]);
      return;
    }
  ...
  function moveScoreSelection(horizontal: number, vertical: number, extend: boolean) {
    ...
    if (!extend) {
      ...
      const seed = cursorAt(current.beat, onset, current.string);
      if (!seed) return;
      scoreCursorRef.current = seed;
      moveScoreCursor(horizontal, vertical);
      return;
    }
    ...
    if (horizontal) {
      ...
      for (let count = 0; beat && count < 128; count += 1) {
        if (beat.voice.index === current.beat.voice.index && beat.voice.bar.staff === current.beat.voice.bar.staff) {
  ...
  function deleteSelectedNotes() {
    ...
    for (const note of selected) {
      ...
      if (target) {
        ...
        if (!target.event.notes.length) target.event.rest = true;
      }
      markRecognitionChanged(note);
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      if (event.key === "Delete" || event.key === "Backspace") { event.preventDefault(); if (!deleteRestAtScoreCursor()) deleteSelectedNotes(); return; }
  ...
  const cursorIsRestColumn = scoreCursor?.kind === "rest";
  const cursorIsEntityRest = Boolean(scoreCursor?.beat.isRest && !scoreCursor.beat.isEmpty);
  const activeDuration = cursorIsEntityRest ? beatBaseDurationEighths(scoreCursor!.beat) : scoreCursor ? entryDuration : selectedBeatDuration;
  const firstMeasure = recognition?.summary.start_measure ?? 1;
  const focusedBar = scoreRef.current?.tracks[0]?.staves[0]?.bars[focusedMeasure - firstMeasure];
  const focusedRhythm = focusedBar ? barRhythm(focusedBar) : null;
  ...
  return (
    <div className="alpha-player score-studio">
      ...
      <div className="score-structure-toolbar">
        ...
        {scoreCursor && !cursorIsRestColumn && <div className="score-string-picker" aria-label="当前编辑弦"><span>弦</span>{Array.from({ length: cursorStringCount }, (_, index) => index + 1).map((string) => <button type="button" key={string} className={cursorVisualString === string ? "active" : ""} onClick={() => selectCursorString(string)}>{string}</button>)}</div>}
        {focusedRhythm && <div className={`measure-rhythm-readout ${focusedRhythm.invalid ? "invalid" : "complete"}`} aria-label={`第 ${focusedMeasure} 小节拍数`}><span>第 {focusedMeasure} 小节</span><strong>{formatQuarterBeats(focusedRhythm.used)} / {formatQuarterBeats(focusedRhythm.capacity)}</strong><small>拍</small></div>}
      </div>
      ...
      <div className="studio-keyboard-hint">
        ...
        <span><kbd>Space</kbd> 播放/暂停</span><span><kbd>拖框</kbd> 按节奏列多选</span><span><kbd>右键</kbd> 选择整列休止</span><span><kbd>Ctrl/⌘ 点击</kbd> 离散多选</span><span><kbd>Shift 点击/方向键</kbd> 连续选择</span><span><kbd>0–9</kbd> 写入品位</span><span><kbd>Enter</kbd> 确认透明休止</span><span><kbd>Delete</kbd> 实体休止变透明</span><span><kbd>.</kbd> 单附点开关</span><span><kbd>Ctrl/⌘ .</kbd> 双附点</span><span><kbd>↑↓</kbd> 当前列逐弦</span><span><kbd>←→</kbd> 前后可见节拍列</span><span><kbd>Alt ↑↓</kbd> 保持音高换弦</span><span><kbd>Ctrl/⌘ S</kbd> 保存</span>
      </div>
      ...
      <div ref={scrollPanelRef} className="score-canvas-panel">
        <div className="score-canvas-content">
          ...
          <div className="score-selection-layer" aria-hidden="true">
            ...
            {dragSelectionBox && <i className="score-lasso" style={{ left: dragSelectionBox.x, top: dragSelectionBox.y, width: dragSelectionBox.width, height: dragSelectionBox.height }} />}
